...
public class DelayedRenderCallManager {
    ...
    public void execRenderCalls() {
        ...
        if(!nametagRenderCalls.isEmpty()) {
            ...
            buffer.bind();
            RenderSystem.setProjectionMatrix(usedProjectionMatrix, VertexSorting.DISTANCE_TO_ORIGIN);
            for(Runnable run : nametagRenderCalls) {
                run.run();
            }
            buffer.bindEnd();
            nametagRenderCalls.clear();
            RenderSystem.setProjectionMatrix(backupProjectionMatrix, VertexSorting.DISTANCE_TO_ORIGIN);
        }
    }
